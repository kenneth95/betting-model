...
# =============================================================================
#
# NOTE: This script is kept as a quick single-split sanity check comparable
# to the original prototype. It is NOT the primary evaluation — a single
# 380-match test season cannot distinguish real edge from noise (confirmed
# by walk_forward.py: this exact season, 2025/26, turned out to be the best
# of the 9 evaluable seasons for both models, and the walk-forward pooled
# ROI across all seasons is not statistically distinguishable from zero).
# Use src/walk_forward.py for anything that informs a real decision.
#
# Feature lists, RANDOM_STATE and STAKE now come from config.py. The
# training window below is also now aligned with train_models.py's
# validation boundary (previously this script trained only through
# 2024-05-19, silently discarding the entire 2024/25 season that
# train_models.py used as its validation set).

DATA_PATH = cfg.MODEL_DATA_PATH
OUTPUT_PATH = cfg.MODELS_DIR / "backtest_results.csv"
DETAIL_OUTPUT_PATH = cfg.MODELS_DIR / "backtest_model_c_5pct_ev.csv"

RANDOM_STATE = cfg.RANDOM_STATE
STAKE = cfg.STAKE

EV_THRESHOLDS = cfg.EV_THRESHOLDS + [0.20]
...
# =============================================================================
#
# FIXED: this previously trained only through 2024-05-19, which silently
# excluded the entire 2024/25 season (train_models.py's validation season)
# from training. Since this script has no separate validation step, that
# season belongs in training here. We now use epl_model_data.csv's own
# "season" column directly instead of hardcoded date strings, so this can
# never drift from build_features.py's season labels again.
#
# Training:
#   2016/17 through 2024/25 (9 seasons)
#
# Test:
#   2025/26 (held out, never touched during training)
#
# =============================================================================
...
# Safety checks
if len(train_df) != 3420:

    raise ValueError(
        f"Expected 3,420 training matches, "
...
prediction_path = cfg.MODELS_DIR / "test_predictions_2025_26.csv"
